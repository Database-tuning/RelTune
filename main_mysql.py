...
from data import load_mysql_data
...
if __name__ == "__main__":
    ...
    param_names, embeddings, desc_embeddings, _ = build_parameter_embeddings(
        "/Description_Embeding_fixed.json"
    )

    edge_index = build_relational_graph(embeddings, similarity_threshold=0.75)
    visualize_graph(embeddings, edge_index, param_names, title="MySQL Parameter Graph (Cosine >= 0.75)")

    scaled_config, scaled_metrics, scaler, metric_scaler = load_mysql_data(
        "configs.csv", "metrics.csv"
    )
    ...
    model = GNN_Autoencoder_With_Surrogate(out_config_dim=138).to(device)
